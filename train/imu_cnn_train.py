...
from imu_cnn_model import IntentCNNTrainer, IntentCNNTuner
...
def train_and_evaluate_imu_cnn(
    ...
    checkpoint_path: str | None = "checkpoints/best_intent_cnn.pt",
    *,
    n_trials: int = 50,
    ...
    """Tune and refit a single-window IMU CNN without accessing test data."""
    ...
    return run_training_experiment(
        ...
        tuner_factory=lambda train, val, search: IntentCNNTuner(
            train, val, in_channels=6, num_classes=7, search_space=search
        ),
        trainer_factory=IntentCNNTrainer,
        config=config,
